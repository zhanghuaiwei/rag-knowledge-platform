package com.ragkb.service.interfaces.config;
 ...
 *
 * <ul>
 *   <li>{@code RAGKB_AUTH_MODE=form}（默认，开发/演示）：账号密码表单登录。
 *       开发用内存用户（{@code RAGKB_DEV_USERNAME / RAGKB_DEV_PASSWORD / RAGKB_DEV_ROLES}），
 *       登录端点为 {@code POST /api/v1/auth/login}（JSON）。</li>
 *   <li>{@code RAGKB_AUTH_MODE=oidc}（生产）：OIDC Authorization Code，由企业 IdP 承载认证，
 *       配置见 {@code RAGKB_OIDC_CLIENT_ID / RAGKB_OIDC_CLIENT_SECRET / RAGKB_OIDC_ISSUER_URI}。</li>
 * </ul>
 *
 * <p>会话方案：BFF 式 HttpOnly 会话 cookie（JSESSIONID），前端不持久化长期 token。
 * 鉴权未通过时 API 统一返回 JSON 401（{@code E-1001}）。
 *
 * <p>⚠️ 脚手架说明：CSRF 暂未启用（当前无跨站写路径的变更面）。生产 cookie 会话必须启用
 * CSRF 保护（BFF 走同源 + SameSite，或按 03-详细设计 §4 引入 double-submit token），
 * 属人工实现点。
 */
@Configuration
@EnableWebSecurity
    ...
    public SecurityFilterChain formFilterChain(
        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/v1/auth/login", "/api/v1/ping", "/actuator/**", "/error").permitAll()
