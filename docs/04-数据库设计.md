...
erDiagram
    ...
    sys_user ||--o{ user_credential : "local credential"
...
| 领域 | 表 | 关键职责 |
|---|---|---|
| 租户与身份 | `sys_tenant`、`sys_user`、`identity_account`、`identity_provider`、`tenant_member`、`tenant_member_role`、`sys_org`、`sys_user_org`、`user_credential`¹ | 全局身份、多身份源、多租户成员、多个组织、租户角色、本地账号凭据 |

> ¹ `user_credential` 由迁移 `deploy/ddl/migrations/V0.4__local_user_credentials.sql` 增量建立，不在 `init.sql` 48 张表基线内；新装执行 init.sql 后再执行 V0.4。
...
- 本地账号（form 登录）：`user_credential.username` 按 `lower(username)` 全局唯一，`sys_user` 不保存密码；
  `user_credential` 只存 BCrypt hash 与凭据策略（失败计数/锁定/过期）。`sys_user` 仍是全局身份唯一真相。
...
- `read` 与 `read_at` 由 mark-read 端点原子更新；未读通知按 `(tenant_id, user_id, create_time DESC)` 排序展示。
...
> 中间件（PostgreSQL/Redis/MinIO）可用 [`deploy/compose/docker-compose.yml`](../deploy/compose/docker-compose.yml) 一键拉起；PostgreSQL 首次启动（空数据卷）自动执行本脚本完成初始化，两个业务角色密码经 `deploy/compose/.env` 的 `RAGKB_APP_PASSWORD` / `RAGKB_MIGRATOR_PASSWORD` 注入。独立/托管 PostgreSQL 的手动执行命令见 `docs/09-部署运维指南.md`。目标为 PostgreSQL 16+。
