...
> **权威边界**：本文定义领域模型、约束和迁移策略；精确表名、列名、类型、索引与外键以 [`deploy/ddl/01-schema.sql`](../deploy/ddl/01-schema.sql) 为准；请求/响应字段仍以 OpenAPI 为准。  
...
| 辅助能力 | `tag`、`document_tag`、`user_favorite`、`webhook_subscription`、`webhook_delivery` | 标签、收藏、Webhook 配置与投递状态 |
...
[`deploy/ddl/04-enable-tenant-rls.sql`](../deploy/ddl/04-enable-tenant-rls.sql) 会对 `sys_tenant` 和所有带 `tenant_id` 的业务表启用策略。启用前必须验证：
...
```bash
# 1. DBA：创建角色和数据库，密码用 psql 变量或受控临时文件传入
psql -U postgres -d postgres \
  -v ragkb_app_password='***' \
  -v ragkb_migrator_password='***' \
  -f deploy/ddl/00-create-database.sql

# 2. migrator：创建 v0.2 Schema 与最小安全种子
psql -U ragkb_migrator -d ragkb -f deploy/ddl/01-schema.sql
psql -U ragkb_migrator -d ragkb -f deploy/ddl/02-seed-data.sql

# 3. 完成应用租户上下文测试后，按需启用 RLS
psql -U ragkb_migrator -d ragkb -f deploy/ddl/04-enable-tenant-rls.sql
```

种子文件只创建本地私有化结构自检租户和一个 `DRAFT/待部署校验`性质的模型档案，不创建密码、管理员、API Key、身份源或模型密钥。生产控制面必须创建真实租户和身份管理员。

### 9.2 v0.1 存量环境

存量升级使用 Expand → Migrate → Contract：

1. **预检**：备份与恢复演练；统计租户、用户、知识库、版本、引用和失败任务；确认对象与索引可重建。
2. **Expand**：执行 [`03-v0.2-enterprise-generalization.sql`](../deploy/ddl/03-v0.2-enterprise-generalization.sql)，只增加租户属性、并行身份关系、索引档案、治理列、幂等与 outbox 基础。
...
`undo_log.sql` 已变为无副作用的废弃提示。只有确认所有旧实例与 Seata 全局事务已结束、备份和审批完成后，运维才能单独处理历史 `undo_log`。
...
- 47 张新装表存在，所有租户业务表均带 `tenant_id` 或明确属于全局身份域。
...
- `03` 是非破坏性 Expand 基线，不替代每个生产环境的容量评估、数据映射报告和 Contract 迁移单。
