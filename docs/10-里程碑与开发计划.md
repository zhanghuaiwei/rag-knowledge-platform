...
- [ ] W1 脚手架：Maven 多模块（server + starter）、Compose 中间件、建库脚本 `deploy/ddl/init.sql`（`04§3` 全部 DDL）
...
- [ ] Seata TC 部署 + `@GlobalTransactional` 事务边界（`03§6.1`）+ undo_log 表（`04§3.16`）—— ⚠️ v0.2 已弃用 Seata，改用「本地事务 + outbox + 幂等」，undo_log 不再创建，见 `04-数据库设计.md` §9
...
**风险**：Seata 与 MyBatis-Plus 多租户插件叠加 → 验证 undo_log 与租户过滤共存（⚠️ v0.2 已弃用 Seata，见上）
