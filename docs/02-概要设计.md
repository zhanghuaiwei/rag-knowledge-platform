...
`docs/api/server.openapi.yaml` 已升级为 v0.2 草稿（评审中未冻结，89 路径/113 操作，覆盖现有 controller 全部路径）；`docs/api/rag-engine.openapi.yaml` 仍为 v0.1，待同步。契约冻结前仍需完成：

- 移除 password grant，定义 OIDC/BFF 登录、登出和 tenant 切换边界（form 登录仅作开发/演示模式）。
...
**实现现状**：web 产品化页面与 service 接口入口骨架已按本概要实现（mock 数据层 + HTTP transport），但业务用例仍为 `NotYetImplemented` stub（501 `E-9998`）。契约冻结前不得把本概要中的候选字段当成冻结 API，也不把 mock/stub 描述为业务验收通过。
